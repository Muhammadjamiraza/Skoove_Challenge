...
for f in csv_files:
    ...
    for index, row in metadata.iterrows():
        ...
        #Extracting features directly from file
        ...
        # Writing the features in .txt file        
        ...
        # Extracting midi features from .wav file by feeding into the model
        act = proc(file_1)
        adsr_1 = adsr(act)
        # Creating List of extracted features
        for audio in adsr_1:
        ...
        # Comparing both values to get the validation metrcs    
        ...
        # Erasing the .txt files
        open('reference.txt', 'w').close()
        open('estimate.txt', 'w').close()
    # Taking the transpose to get the values of same parameter in one line    
    results = np.array(results, dtype = 'object').T.tolist() 
    # Creating the separate dimension for every dataset to visualize separately
    def extractDigits(lst):
        return [[el] for el in lst]
    result = extractDigits(results)
    # Appending the values of same parameters of every file
...
# Visualization of results by plotting a box plot
